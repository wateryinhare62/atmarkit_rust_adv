...
pub async fn api_index_withdb(pool: web::Data<Pool<Sqlite>>, 
    ...
    info!("Called index API DB version");
    let param = query.into_inner();
    let pool = pool.get_ref();
    let posts = data::get_all_withdb(pool.clone()).await;
    let mut ary = Vec::new();
    for item in &posts {
        ary.push(item.clone());
    }
...
#[get("/posts/{id}")]
pub async fn api_show_withdb(pool: web::Data<Pool<Sqlite>>, 
        info: web::Path<i32>, query: web::Query<Queries>) -> impl Responder {
    info!("Called show API DB version");
    let info = info.into_inner();
    let param = query.into_inner();
    let pool = pool.get_ref();
    let post = data::get_withdb(pool.clone(), info).await;
    let response = ApiResponse {
        status: "OK".to_string(),
        result: ResponseContent::Item(post),
    };
...
#[post("/posts/create")]
pub async fn api_create_withdb(pool: web::Data<Pool<Sqlite>>, 
        params: web::Json<data::Message>) -> impl Responder {
    info!("Called create API DB version");
    let now: DateTime<Local> = Local::now();
    let mut message = data::Message {
        id: 0, 
        posted: now.format("%Y-%m-%d %H:%M:%S").to_string(),
    ...
    let pool = pool.get_ref();
    message = data::create_withdb(pool.clone(), message).await;
    let response = ApiResponse {
        status: "OK".to_string(),
        result: ResponseContent::Item(message),
    };
...
#[put("/posts/update")]
pub async fn api_update_withdb(pool: web::Data<Pool<Sqlite>>, 
        params: web::Json<data::Message>, query: web::Query<Queries>) -> impl Responder {
    info!("Called update API DB version");
    let message = data::Message {
        id: params.id, 
        posted: params.posted.clone(),
        sender: params.sender.clone(),
    ...
    let pool = pool.get_ref();
    data::update_withdb(pool.clone(), &message).await;
    let response = ApiResponse {
        status: "OK".to_string(),
        result: ResponseContent::Item(message),
    };
...
#[delete("/posts/{id}/delete")]
pub async fn api_destroy_withdb(pool: web::Data<Pool<Sqlite>>, 
        info: web::Path<i32>, query: web::Query<Queries>) -> impl Responder {
    info!("Called destroy API DB version");
    let info = info.into_inner();
    let param = query.into_inner();
    let pool = pool.get_ref();
    data::remove_withdb(pool.clone(), info).await;
    let response = ApiResponse {
        status: "OK".to_string(),
        result: ResponseContent::None,
    };
